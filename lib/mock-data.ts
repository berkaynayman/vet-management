...
export const apiClient = {
  ...
  getPets: async (ownerId?: string) => {
    try {
      ...
      return pets.map((pet) => ({
        ...pet,
        owner: mockUsers.find((u) => u.id === pet.owner_id),
      }));
    }
  },
  ...
  createPet: async (petData: Omit<MockPet, "id" | "created_at">) => {
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("Authentication required");
  ...
  updatePet: async (petId: string, updates: Partial<MockPet>) => {
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("Authentication required");
  ...
  getAppointments: async (filters?: { petIds?: string[]; doctorId?: string }) => {
    try {
      ...
      return appointments.map((appointment) => ({
        ...appointment,
        pet: {
          ...mockPets.find((p) => p.id === appointment.pet_id)!,
          owner: mockUsers.find((u) => u.id === mockPets.find((p) => p.id === appointment.pet_id)?.owner_id),
        },
        doctor: mockUsers.find((u) => u.id === appointment.doctor_id),
      }));
    }
  },
  ...
  getAppointment: async (appointmentId: string) => {
    try {
      ...
      const pet = mockPets.find((p) => p.id === appointment.pet_id)!;
      const owner = mockUsers.find((u) => u.id === pet.owner_id)!;
      const doctor = mockUsers.find((u) => u.id === appointment.doctor_id)!;
      const details = mockAppointmentDetails.find((d) => d.appointment_id === appointmentId);
  ...
  createAppointment: async (appointmentData: Omit<MockAppointment, "id" | "created_at">) => {
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("Authentication required");
  ...
  updateAppointment: async (appointmentId: string, updates: Partial<MockAppointment>) => {
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("Authentication required");
  ...
  // Appointment Details
  createOrUpdateAppointmentDetails: async (appointmentId: string, detailsData: Omit<MockAppointmentDetails, "id" | "created_at" | "appointment_id">) => {
    try {
      const token = localStorage.getItem("authToken");
      if (!token) {
        throw new Error("Authentication required");
      ...
      // Fallback to mock implementation
      const existingDetailsIndex = mockAppointmentDetails.findIndex(
        (d) => d.appointment_id === appointmentId
      );
      
      if (existingDetailsIndex > -1) {
        // Update existing details
        mockAppointmentDetails[existingDetailsIndex] = {
          ...mockAppointmentDetails[existingDetailsIndex],
          ...detailsData,
        };
        return mockAppointmentDetails[existingDetailsIndex];
      } else {
        // Create new details
        const newDetails: MockAppointmentDetails = {
          ...detailsData,
        ...
        mockAppointmentDetails.push(newDetails);
        return newDetails;
      }
    }
  },
  ...
  getStats: async () => {
    try {
      ...
      return {
        ...
        totalOwners: mockUsers.filter((u) => u.role === "pet_owner").length,
      };
    }
  },
}
